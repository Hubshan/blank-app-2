...
MCQS = [
('1NF mainly requires:', ['Atomic values','No foreign keys','No primary key','One row only'],0),
('Partial dependency is mainly removed by:', ['1NF','2NF','3NF','BCNF'],1),
('Transitive dependency is mainly removed by:', ['1NF','2NF','3NF','BCNF'],2),
('BCNF requires every determinant to be a:', ['Foreign key','Non-key attribute','Superkey','Tuple'],2),
('A comma-separated list in one cell most directly violates:', ['1NF','2NF','3NF','BCNF'],0),
('2NF assumes the relation is already in:', ['1NF','3NF','BCNF','4NF'],0),
...
('A determinant is the left side of a:', ['Tuple','Functional dependency','Foreign key','Primary key only'],1),
('A relation can be in 3NF but not BCNF:', ['Yes','No','Only without a key','Only in 1NF'],0),
]
...
def get_problems():
    if st.session_state.mode=='All Problems': return PROBLEMS
    return [p for p in PROBLEMS if p[0]==st.session_state.mode]

def save_attempt(level,ptype,q,answer,correct,points):
    if db:
        try:
            db.table('normalization_attempts').insert({'player':st.session_state.player,'level':level,'problem_type':ptype,'question':q,'selected_answer':answer,'correct':correct,'points':points}).execute()
        except Exception: pass

def save_score():
    if not db or st.session_state.saved: return
    total=len(st.session_state.attempts); correct=sum(x['correct'] for x in st.session_state.attempts); accuracy=round(correct/total*100) if total else 0
    try:
        db.table('normalization_scores').insert({'player':st.session_state.player,'score':st.session_state.score,'correct':correct,'total':total,'accuracy':accuracy}).execute(); st.session_state.saved=True
    except Exception: pass

def leaderboard():
    if not db: return pd.DataFrame()
    try:
        r=db.table('normalization_scores').select('player,score,correct,total,accuracy,created_at').order('score',desc=True).order('created_at',desc=False).limit(20).execute()
...
st.markdown('''<style>.hero{padding:1.2rem 1.4rem;border-radius:18px;color:white;background:linear-gradient(135deg,#1d4ed8,#7c3aed);margin-bottom:1rem}.problem{padding:1rem 1.2rem;border-left:6px solid #7c3aed;border-radius:10px;background:#faf7ff}</style>''',unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if not st.session_state.started:
        ...
        if st.button('🚀 Start Game',type='primary',use_container_width=True):
            if not name.strip(): st.warning('Please enter your name.')
                ...
                st.session_state.player=name.strip();st.session_state.mode=mode;st.session_state.started=True;st.session_state.idx=0;st.session_state.score=0;st.session_state.attempts=[];st.session_state.saved=False;st.rerun()
    else:
    ...
    st.divider(); st.success('☁️ Shared leaderboard ready') if db else st.warning('☁️ Add Supabase secrets to enable sharing')

if not st.session_state.started:
    st.subheader('🏆 Shared Online Scorecard')
    board=leaderboard()
    if not board.empty:
        board.insert(0,'Rank',range(1,len(board)+1)); board['created_at']=pd.to_datetime(board['created_at']).dt.strftime('%d-%m-%Y %H:%M')
        st.dataframe(board.rename(columns={'player':'Player','score':'Score','correct':'Correct','total':'Total','accuracy':'Accuracy %','created_at':'Time'}),use_container_width=True,hide_index=True)
    else: st.info('No scores yet. Be the first player!')
    st.write('All students using the same deployed app will see the same leaderboard when Supabase is configured.')
    st.stop()

ps=get_problems(); total=len(ps)+len(MCQS); i=st.session_state.idx
if i < len(ps):
    level,ptype,q,opts,ans,why=ps[i]; st.progress(i/total); st.caption(f'Problem {i+1} of {len(ps)} • {level} • {ptype}')
    st.markdown(f"<div class='problem'><b>{ptype}</b><br><br>{q}</div>",unsafe_allow_html=True)
    answer=st.radio('Choose the best solution:',opts,index=None,key=f'p{i}')
    if st.button('✅ Submit Problem',type='primary',use_container_width=True):
        if answer is None: st.warning('Select an answer first.')
        else:
            correct=opts.index(answer)==ans;points=10 if correct else 0;st.session_state.score+=points
            st.session_state.attempts.append({'level':level,'type':ptype,'question':q,'selected':answer,'correct':correct,'points':points})
            save_attempt(level,ptype,q,answer,correct,points)
            st.success(f'✅ Correct! +{points} points.') if correct else st.error(f'❌ Incorrect. Correct answer: {opts[ans]}')
            st.info(why);st.session_state.idx+=1;st.rerun()
elif i < total:
    qi=i-len(ps);q,opts,ans=MCQS[qi];st.progress(i/total);st.caption(f'MCQ {qi+1} of {len(MCQS)}')
    st.markdown(f"<div class='problem'><b>MCQ</b><br><br>{q}</div>",unsafe_allow_html=True);answer=st.radio('Choose one:',opts,index=None,key=f'm{qi}')
    if st.button('✅ Submit MCQ',type='primary',use_container_width=True):
        if answer is None: st.warning('Select an answer first.')
        else:
            correct=opts.index(answer)==ans;points=5 if correct else 0;st.session_state.score+=points
            st.session_state.attempts.append({'level':'MCQ','type':'MCQ','question':q,'selected':answer,'correct':correct,'points':points})
            save_attempt('MCQ','MCQ',q,answer,correct,points)
            st.success(f'✅ Correct! +{points} points.') if correct else st.error(f'❌ Incorrect. Correct answer: {opts[ans]}')
            st.session_state.idx+=1;st.rerun()
else:
    save_score();df=pd.DataFrame(st.session_state.attempts);total_a=len(df);correct=int(df['correct'].sum());accuracy=round(correct/total_a*100) if total_a else 0
    st.balloons();st.success('🎉 Game Complete!');st.header(f'🏆 Scorecard — {st.session_state.player}')
    a,b,c,d=st.columns(4);a.metric('Final Score',st.session_state.score);b.metric('Correct',correct);c.metric('Wrong',total_a-correct);d.metric('Accuracy',f'{accuracy}%')
    st.subheader('🌐 Shared Online Leaderboard')
    board=leaderboard()
    if not board.empty:
        board.insert(0,'Rank',range(1,len(board)+1));board['created_at']=pd.to_datetime(board['created_at']).dt.strftime('%d-%m-%Y %H:%M')
        st.dataframe(board.rename(columns={'player':'Player','score':'Score','correct':'Correct','total':'Total','accuracy':'Accuracy %','created_at':'Time'}),use_container_width=True,hide_index=True)
    else: st.warning('Leaderboard unavailable. Configure Supabase Secrets.')
    st.subheader('📝 My Answer History');st.dataframe(df,use_container_width=True,hide_index=True)
    st.download_button('⬇️ Download My Scorecard',df.to_csv(index=False).encode('utf-8'),f'{st.session_state.player}_normalization_scorecard.csv','text/csv',use_container_width=True)
